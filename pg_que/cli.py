...
async def run_perf_tests(n=10000):
    ...
    print(f'Took {took} to send {n} messages. {took / n} per message.')

    start = time.perf_counter()
    more_messages = True
    while more_messages:
        m = await queue.receive_messages(batch_size=1)
        if not m:
            more_messages = False
    end = time.perf_counter()
    took = end - start

    print(f'Took {took} to receive {n} messages. {took / n} per message.')
